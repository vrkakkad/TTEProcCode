function [pre,push,idx_pre,idx_push] = dispMmode(options,nax,nt,datastruct_pre,datastruct_push,par,gate,edge,mask,dispPar,img_rng)
...
set(ax13,'xcolor',dispPar.txt,'ycolor',dispPar.txt,'fontweight','bold','UserData','pre_ax','xlim',[0 max(datastruct_pre.acqTime)],'ylim',[min(edge)-1 max(edge)+1])
set(cax13,'box','on','linewidth',3,'xcolor','y','ycolor','y','xticklabel',[],'yticklabel',[],'xgrid','on','UserData','copy_pre_ax','xlim',[0 max(datastruct_pre.acqTime)],'ylim',[min(edge)-1 max(edge)+1]);
...
set(ax14,'xcolor',dispPar.txt,'ycolor',dispPar.txt,'fontweight','bold','UserData','push_ax','xlim',[0 max(datastruct_pre.acqTime)],'ylim',[min(edge)-1 max(edge)+1])
set(cax14,'box','on','linewidth',3,'xcolor','r','ycolor','r','xticklabel',[],'yticklabel',[],'xgrid','on','UserData','copy_push_ax','xlim',[0 max(datastruct_pre.acqTime)],'ylim',[min(edge)-1 max(edge)+1]);
