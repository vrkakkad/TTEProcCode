function [pre,push,idx_pre,idx_push] = dispMmode(options,nax,nt,datastruct,par,gate,edge,mask,dispPar,img_rng)

if (isfield(datastruct,'disp_mf_pre') && isfield(datastruct,'disp_mf_push'))
...
hold on
l5 = plot(linspace(datastruct.acqTime(1),datastruct.acqTime(end),length(gate)),gate(:,1),'g','Linewidth',2);
l6 = plot(linspace(datastruct.acqTime(1),datastruct.acqTime(end),length(gate)),gate(:,2),'g','Linewidth',2);
...
set(cax13,'box','on','linewidth',3,'xcolor','y','ycolor','y','xticklabel',[],'yticklabel',[],'xgrid','on','UserData','copy_pre_ax','xlim',[0 max(datastruct.acqTime)],'ylim',[min(edge)-1 max(edge)+1]);
...
hold on
l7 = plot(linspace(datastruct.acqTime(1),datastruct.acqTime(end),length(gate)),gate(:,1),'g','Linewidth',2);
l8 = plot(linspace(datastruct.acqTime(1),datastruct.acqTime(end),length(gate)),gate(:,2),'g','Linewidth',2);
...
set(cax14,'box','on','linewidth',3,'xcolor','r','ycolor','r','xticklabel',[],'yticklabel',[],'xgrid','on','UserData','copy_push_ax','xlim',[0 max(datastruct.acqTime)],'ylim',[min(edge)-1 max(edge)+1]);
