...
void sectorDOUBLE(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    mwSize *out_dims;
    double *in1, *x, *y, *r, *phi, *u, *v;
    double *out, *ax, *lat;
    double min_in, min_u, max_u, min_v, max_v;
        
    // Initialize input variables //
    in       = (double *)mxGetPr(prhs[1]);
    sz1      = mxGetM(prhs[1]);
    sz2      = mxGetN(prhs[1]);
    sz3      = 1;
    ...
    // Pre-allocate memory
    in1 = (double *)mxMalloc(sizeof(double)* sz1* sz2);
    x   = (double *)mxMalloc(sizeof(double)*osz1*osz2);
    y   = (double *)mxMalloc(sizeof(double)*osz1*osz2);
    r   = (double *)mxMalloc(sizeof(double)*osz1*osz2);
    phi = (double *)mxMalloc(sizeof(double)*osz1*osz2);
    u   = (double *)mxMalloc(sizeof(double)*osz1*osz2);
    v   = (double *)mxMalloc(sizeof(double)*osz1*osz2);
    ...
    plhs[1] = mxCreateNumericMatrix(1, osz2, mxDOUBLE_CLASS, mxREAL);
    plhs[2] = mxCreateNumericMatrix(1, osz1, mxDOUBLE_CLASS, mxREAL);
    
    // Set up output variables
    out = (double *)mxGetPr(plhs[0]);
    ax  = (double *)mxGetPr(plhs[1]);
    lat = (double *)mxGetPr(plhs[2]);

    // Calculate ax and lat and convert to mm
    for(i = 0; i < osz2; i++)
        ax[i] = (x_min+incx*i+(double)apex)*100;
    for(i = 0; i < osz1; i++)
